...
const XplafesDetail = ({ navigation }) => {
  ...
  //  according to figma
    let fDate = tempDate.getDate() + "/" + (tempDate.getMonth() + 1) + " /" + tempDate.getFullYear() + "/" + tempDate.getHours() + ":" + tempDate.getMinutes() ; // 1 is index

    // let fTime = "Hours: " + tempDate.getHours() + " | Minutes: " + tempDate.getMinutes();
// set according to figma
  ...
  const sendToBackend = () => {
    ...
    if (message == '') {
      ...
      AsyncStorage.getItem('user').then(
        data => {
          fetch('https://women-safety-app-backend-production.up.railway.app/setmessage', {
            method: 'post',
            headers: {
